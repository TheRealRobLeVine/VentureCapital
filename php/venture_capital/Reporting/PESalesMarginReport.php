...
class PESalesMarginReport extends Report {
	...
	public function __construct($userID=null) {
		...
		foreach($plan->getCategories() as $category) {
				...
				$unitCost = ($singleCOGS != null) ? $singleCOGS->getUnitCost() : 0;
				$row .= (new HTMLTableCell(Globals::numberFormat($unitCost, Globals::NUMBER_FORMAT_TYPE_PRICE_FRACTION, true)))->getHTML();
				
				foreach($plan->getChannels(false) as $channel) {
					$channelID = $channel->getID();
					$priceAndMargin = $plan->getPriceAndMarginByCategoryAndChannel($categoryID, $channelID);
					if (null != $priceAndMargin) {
