<?php
	...
	function salesProjectionsReport($atts = [], $content = null, $tag = '') {
		...
		foreach($projections as $projections) {
			...
			foreach($projections->units_data as $units_data) {
				...
				$units_data->growth = getGrowthByChannelOrCategory($units_data->y_axis_ID, false, $userID);
				$totalUnits = 0;
				$subtotalUnits = array();
				for($i=$planStartYear;$i<($planStartYear+$planLength);$i++) {
	...
	*   $userID - 
	*/
	function getGrowthByChannelOrCategory($channelOrCategoryID, $useChannel, $userID) {
		error_log("getGrowthByChannelOrCategory - ID: $channelOrCategoryID");
		error_log("getGrowthByChannelOrCategory - isChannel: $useChannel");
		if (true) {
			$percentGrowthChildFormID = FrmForm::get_id_by_key("n6kck");
			$percentGrowthYearFieldPrefix = "financialplan-salesforecasting_growth_channel_year_";
		}
		else {
			$percentGrowthChildFormID = FrmForm::get_id_by_key("lqnwq");
			$percentGrowthYearFieldPrefix = "financialplan-salesforecasting_growth_category_year_";
		}
		
		$growthEntries = FrmEntry::getAll(array('it.form_id' => $percentGrowthChildFormID, 'it.user_id' => $userID), '', '', true);
		$growthRates = array();
		foreach ($growthEntries as $growthEntry) {
				for($i=1;$i<=PLAN_MAX_YEARS;$i++) {
					$fieldKey =  $percentGrowthYearFieldPrefix . $i;
